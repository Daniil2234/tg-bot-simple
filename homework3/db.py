...
def add_note(user_id: int, text: str) -> int:
    """Добавить заметку. Возвращает ID заметки или 0 если лимит превышен"""
    with _connect() as conn:
        # Проверяем лимит (максимум 50 заметок)
...
def list_notes(user_id: int, limit: int = 10):
    """Список последних заметок пользователя"""
    with _connect() as conn:
        cur = conn.execute(
...
def update_note(user_id: int, note_id: int, text: str) -> bool:
    """Обновить заметку. Возвращает True если успешно"""
    with _connect() as conn:
        cur = conn.execute(
            """UPDATE notes
            SET text = ?
...
def delete_note(user_id: int, note_id: int) -> bool:
    """Удалить заметку. Возвращает True если успешно"""
    with _connect() as conn:
...
def get_note_count(user_id: int) -> int:
    """Получить количество заметок пользователя"""
    with _connect() as conn:
        cur = conn.execute(
            "SELECT COUNT(id) as count FROM notes WHERE user_id = ?",
            (user_id,)
        )
...
def list_all_notes(user_id: int):
    """Получить все заметки пользователя (для экспорта)"""
    with _connect() as conn:
        cur = conn.execute(
            """SELECT id, text, created_at
            FROM notes
...
def get_combined_stats(user_id: int):
    ...
    with _connect() as conn:
        # Количество заметок и суммарное количество символов
        ...
        if notes_row and notes_row['count'] > 0:
            ...
            stats['total_chars'] = notes_row['chars'] if notes_row['chars'] else 0
